const { pinyin, pinyinEn, initEnglish } = require('../');

// Use the same text as english-benchmark.js
...
// Repeat text to make it substantial
const longText = text.repeat(10); 

console.log('Text length:', longText.length);

// Warmup and init
pinyin(longText.substring(0, 100));
initEnglish();
pinyinEn(longText.substring(0, 100));

const ITERATIONS = 20;

console.log(`Running benchmark with ${ITERATIONS} iterations...`);

// Test pinyin (without English)
const startPinyin = process.hrtime();
for (let i = 0; i < ITERATIONS; i++) {
  pinyin(longText);
}
const endPinyin = process.hrtime(startPinyin);
const timePinyin = (endPinyin[0] * 1000 + endPinyin[1] / 1e6) / ITERATIONS;
console.log(`pinyin (without English): ${timePinyin.toFixed(3)} ms/op`);

// Test pinyinEn (with English)
const startPinyinEn = process.hrtime();
for (let i = 0; i < ITERATIONS; i++) {
  pinyinEn(longText);
}
const endPinyinEn = process.hrtime(startPinyinEn);
const timePinyinEn = (endPinyinEn[0] * 1000 + endPinyinEn[1] / 1e6) / ITERATIONS;
console.log(`pinyinEn (with English): ${timePinyinEn.toFixed(3)} ms/op`);

const slowdown = ((timePinyinEn - timePinyin) / timePinyin) * 100;
console.log(`Performance difference: pinyinEn is ${slowdown.toFixed(2)}% slower than pinyin`);
