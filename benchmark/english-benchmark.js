const { pinyinEn } = require('../');
const fs = require('fs');
const path = require('path');

// We use the same text as accuracy.js for consistency
...
console.log('Running English dictionary benchmark...');
console.time('pinyinEn');
const result = pinyinEn(text);
console.timeEnd('pinyinEn');
...
result.forEach(item => {
  totalSegments++;
  // Check if it's Chinese (rudimentary check or using segment info if available)
  // Here we assume if pinyin is different from zh, it's likely Chinese or handled specially
  // Better check: use a regex for Chinese characters
  if (/[\u4e00-\u9fa5]/.test(item.zh)) {
    chineseSegments++;
    if (item.en && item.en.length > 0) {
      englishFound++;
    } else {
      if (noEnglish.length < 10) { // Keep first 10 for sample
        noEnglish.push(item.zh);
      }
    }
  }
});
...
console.log('English definitions found:', englishFound);
console.log('Coverage:', ((englishFound / chineseSegments) * 100).toFixed(2) + '%');

if (noEnglish.length > 0) {
  console.log('Sample words without English definitions:', noEnglish.join(', '));
}
